...
Output is an IMAGE batch of float32 frames -- HDR-safe (values may exceed 1.0,
which is the whole point) -- plus a synthesised VIDEO socket built from
:func:`ltxv_api.video_type.make_video_from_components` so downstream
``SaveVideo`` (or ``AM Write Video``) can reach the data without the artist
having to plumb the IMAGE socket separately.
...
from .. import exr_loader, tensors, video_type
from ..client import (
    LTXVClient,
    LTXVError,
    JobResult,
...
from ._common import silent_audio_stub
...
class LTXVAPIVideoToVideoHDR:
            ...
            "required": {
                "fps_for_encoding": ("FLOAT", {
                    ...
                    "tooltip": (
                        ...
                        "VIDEO socket is wired (the VIDEO's own fps wins). The "
                        "output EXR frame rate is reported separately on the "
                        "frame_rate output socket."
                    ),
                }),
                "output_fps": ("FLOAT", {
                    "default": 24.0, "min": 1.0, "max": 60.0, "step": 0.001,
                    "tooltip": (
                        "FPS to tag on the synthesised VIDEO output socket. The "
                        "EXR ZIP doesn't carry a time base -- this is what the "
                        "downstream SaveVideo / AM Write Video will encode at."
                    ),
                }),
            ...
            "optional": {
                ...
                "video_url": ("STRING", {
                    ...
                    "tooltip": (
                        ...
                        "over the `video` / `image` sockets. Recommended for "
                        "inputs that would balloon the base64 request body."
                    ),
                }),
            },
        }
    ...
    OUTPUT_TOOLTIPS = (
        ...
        "Synthesised ComfyUI VIDEO built from the EXR batch + output_fps. No source "
        "MP4 -- this socket is only useful when wiring directly into SaveVideo / "
        "AM Write Video; for HDR-aware export prefer the IMAGE socket and an "
        "EXR-aware writer (AM Write Image with EXR output).",
        "Silent audio stub. The HDR endpoint produces no audio.",
    ...
    def execute(
        ...
        try:
            ...
            video_obj = video_type.make_video_from_components(tensor, float(output_fps))

            info_str = (
                f"{width}x{height} {n_frames} EXR frames @ {output_fps:.3f}fps "
                f"(job={terminal.id})"
            )
            return (tensor, video_obj, audio, info_str, width, height, float(output_fps), n_frames)
        finally:
