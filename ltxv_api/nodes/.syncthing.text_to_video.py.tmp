...
Wraps ``POST /v1/text-to-video``. Returns an MP4 from the LTX endpoint and
exposes it as both an IMAGE batch (for nodes that work with frames) and a
native ComfyUI VIDEO socket (for SaveVideo / GetVideoComponents / partner
API nodes).
...
class LTXVAPITextToVideo:
    ...
    OUTPUT_TOOLTIPS = (
        ...
        "Audio track from the MP4 (silent stub when generate_audio=Off or "
    ...
    def execute(
        ...
        return (image, video_obj, audio, info_str, width, height, float(decoded_fps), n_frames)
...
def empty_result() -> tuple:
    return (
        empty_image_tensor(), None, silent_audio_stub(), "(empty)",
        64, 64, 0.0, 1,
    )
