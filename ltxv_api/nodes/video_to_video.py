...
* ``video`` (VIDEO socket) -- preferred when chained from another VIDEO-emitting
  node (Read Video, another LTXV API node).
* ``image`` (IMAGE batch) -- a multi-frame batch is encoded to MP4 first.
...
class LTXVAPIVideoToVideo:
            ...
            "optional": {
                ...
                "video_url": ("STRING", {
                    ...
                    "tooltip": (
                        ...
                        "wins over the `video` / `image` sockets and the request "
                        "uses the URL verbatim. Recommended for inputs too large "
                        "for base64 (10s of MB+)."
                    ),
                }),
            },
        }
    ...
    OUTPUT_TOOLTIPS = (
        ...
        "Audio track from the MP4 (silent stub when mode=replace_video and the input had no audio).",
